...
class Classifier {
  ...
  // 같은 0.40이 최적이었다.
  //
  // T79(2026-09-07): 사용자가 실기기에서 "횡단보도 위와 앞 판단이 안 된다"고
  // 보고해 approach만 **0.40 -> 0.55**로 올렸다. 누수 없는 5-fold CV 865장
  // 실측(`train/state_angle_consistency.py`)에서 임계값별 결과:
  //   0.40  recall 85.9% / precision 73.9% / 위->앞 오판 4.4% / 무판정 3.4%
  //   0.55  recall 83.8% / precision 81.0% / 위->앞 오판 2.7% / 무판정 5.4%
  //   0.60  recall 83.8% / precision 84.4% / 위->앞 오판 2.3% / 무판정 6.1%
  // precision을 7.1%p 얻고 recall을 2.1%p 내주는 교환이다. approach는 "앞에
  // 횡단보도"라는 안내라 없는데 있다고 말하는 쪽(precision 부족)이 더 거슬리고,
  // 놓쳐도 몇 걸음 뒤 다시 잡히므로 이 교환을 택했다. 0.60은 무판정이 6.1%까지
  // 올라 침묵이 늘어나므로 0.55에서 멈췄다.
  //
  // 남은 한계: **앞 -> 위로 오판하는 9.2%는 임계값으로 고쳐지지 않는다**
