...
- NCHW 입력 텐서
- softmax(logits) -> 확률
- 클래스별 임계값 판정: front=0.50, left/right=0.55, none=0.50, approach=0.50(T51 잠정값)
...
T51 참고: classifier.dart는 이제 5-class(none/approach/front/left/right)를 알고 있으며
(docs/Tasks.md T51), 이 평가 스크립트의 기본 임계값도 그 실제 값과 동기화되어 있다.
...
# T51: "approach"(인도 위, 앞에 진입할 횡단보도가 보임) 클래스 추가 -> 5-class.
# 주의(정확성 필수): torchvision ImageFolder는 하위 폴더명을 알파벳순으로 정렬해
# class_to_idx를 부여하므로(train_model.py get_loaders()), 폴더에 숫자 접두사를
# 붙여 알파벳순 = 의도한 인덱스순이 되도록 했다. 실측값:
#   {'0_none': 0, '1_approach': 1, '2_front': 2, '3_left': 3, '4_right': 4}
# 즉 실제 모델 출력(logits)의 인덱스 순서는 none(0), approach(1), front(2),
# left(3), right(4)이다. 아래 두 리스트는 반드시 같은 인덱스를 가리켜야 하고,
# LABELS는 crosswalk_app/lib/services/classifier.dart의 `_labels`와 정확히
# 일치해야 한다 — "보기 편한" 순서로 바꾸면 로짓을 잘못된 라벨에 매핑하는
# 조용한 버그가 된다.
CLASS_DIRS = ["0_none", "1_approach", "2_front", "3_left", "4_right"]
LABELS = ["none", "approach", "front", "left", "right"]
LABEL_TO_DIR = dict(zip(LABELS, CLASS_DIRS))
IMG_SIZE = 224
...
# T51 신설 — classifier.dart의 _approachThreshold와 동일. 재학습 전 잠정값이다
# (approach는 앱에서 none과 동일하게 침묵 처리되므로 같은 값에서 출발).
...
APPROACH_THRESHOLD = float(os.environ.get("EVAL_APPROACH_THRESHOLD", APPROACH_THRESHOLD))
...
def main():
    ...
    for true_label in LABELS:
        class_dir = TEST_DIR / LABEL_TO_DIR[true_label]
        files = sorted(
        ...
        for f in files:
            ...
            logits = outputs[0][0]  # [1,5] -> [5] (T51: none/approach/front/left/right)
    ...
    print("=" * 70)
    print("T1/T51 모델 정확도 실측 결과 "
          "(5-class: none/approach/front/left/right, model/crosswalk_model.onnx)")
    print("=" * 70)
    print("\n표본 크기: " + ", ".join(f"{lab}={file_counts[lab]}" for lab in LABELS) + "\n")
